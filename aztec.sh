...
# Указываете свои RPC ссылки ниже
ETHEREUM_RPC_URL="https://sepolia.your-rpc-node-address"
CONSENSUS_BEACON_URL="https://your-beacon-node-address"
...
2)
  ...
  echo -ne "${YELLOW}Введите приватный ключ валидатора (без 0x): ${NC}"; read -r OLD_VALIDATOR_PK

  # Получаем адрес для вывода стейка из приватного ключа
  WITHDRAW_ADDR=$(cast wallet address "0x${OLD_VALIDATOR_PK}" 2>/dev/null || true)
  [[ "$WITHDRAW_ADDR" =~ ^0x[0-9a-fA-F]{40}$ ]] || { echo -e "${RED}Не удалось вычислить ETH-адрес для вывода через cast${NC}"; exit 1; }

  # Показываем адрес вывода стейка
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"
  echo -e "${GREEN}Адрес для вывода стейка (WITHDRAW_ADDR):${NC}"
  echo -e "${CYAN}${WITHDRAW_ADDR}${NC}"
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"

  # IP сервера определяем автоматически
  P2P_IP="$(curl -s4 https://ipecho.net/plain || curl -s4 ifconfig.me || echo "")"
  echo -e "${GREEN}Публичный IP сервера (P2P_IP): ${CYAN}${P2P_IP}${NC}"
  ...
  echo -e "${PURPLE}Ищите по адресу аттестера: ${CYAN}${RAW_ETH_FIELD}${NC}"
  echo -ne "${YELLOW}После этого нажмите Enter для продолжения...${NC}"; read -r _

  # --- keystore.json для контейнера ---
...
EOF
  ...
  # --- docker-compose.yml ---
  cat > "$WORKDIR/docker-compose.yml" <<'EOFCOMPOSE'
services:
  aztec-node:
...
3)
  if [ -d "$WORKDIR" ]; then
    cd "$WORKDIR" && docker compose logs -fn 200
  else
...
4)
  if [ -d "$WORKDIR" ]; then
    cd "$WORKDIR" && docker compose restart && echo -e "${GREEN}Перезапущено.${NC}"
  else
    echo -e "${RED}Каталог $WORKDIR не найден.${NC}"
  fi
  ;;
